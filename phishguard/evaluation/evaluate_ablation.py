...
def main():
    ...
    with app.app.test_client() as client:
        for row in rows:
            payload=client.post("/predict",json={"url":row["url"]}).get_json(); full.append(int(payload["prediction"])); rules_only.append(rule_only_prediction(row["url"],payload)); threshold=float(app.model["metadata"].get("decision_threshold",.5)); ai_only.append(int(payload["calibrated_ai_phishing_probability"]>=threshold))
