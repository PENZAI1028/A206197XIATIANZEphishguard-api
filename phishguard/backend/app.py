...
def analyse_url(url):
    ...
    # domains may not impersonate a protected brand, but a strong combination
    # of AI probability, credential/payment keywords, and abnormal URL
    # structure should not be presented as relatively safe.
    suspicious_signal_combination = (
        not official_domain
        and not critical
        and (
            (
                ai_score >= 45
                and keyword_score >= 75
                and structure_score >= 25
            )
            or (
                ai_score >= 50
                and keyword_score >= 40
                and structure_score >= 60
            )
        )
    )

    if suspicious_signal_combination:
        overridden_score = max(risk_score, 45)
        if overridden_score != risk_score:
            applied_overrides.append(
                "combined suspicious-signal floor: final risk raised to at least 45"
            )
        risk_score = overridden_score
